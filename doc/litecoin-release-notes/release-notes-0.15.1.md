Vuicoin Core version *0.15.1* is now available from:

  <https://download.vuicoin.org/vuicoin-0.15.1/>
...
A number of changes to the way Vuicoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 namvui per kB (0.00001 VUI). This is to prepare for dropping the minimum transaction fee to 0.1 namvui per kB (0.0001 VUI) in the next release.

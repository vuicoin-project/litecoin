Vuicoin Core version 0.13.2 is now available from:

  <https://download.vuicoin.org/vuicoin-0.13.2.1/>
...
Please report bugs using the issue tracker at github:
  ...
  <https://github.com/vuicoin-project/vuicoin/issues>
...
updates will be released anymore. Without security updates, using a vuicoin
...
ECDSA signatures inside Vuicoin transactions now use validation using
...
Previous versions of Vuicoin Core had their mempool limited by checking
a transaction's fees against the node's minimum relay fee. There was no
upper bound on the size of the mempool and attackers could send a large
number of transactions paying just slighly more than the default minimum
relay fee to crash nodes with relatively low RAM. A temporary workaround
for previous versions of Vuicoin Core was to raise the default minimum
relay fee.

Vuicoin Core 0.13.2 will have a strict maximum size on the mempool. The
...
Vuicoin Core 0.13.2 also introduces new default policy limits on the length and
...
This allows running vuicoind without having to do any manual configuration.
...
Vuicoin Core has a heuristic 'priority' based on coin value and age. This
calculation is used for relaying of transactions which do not pay the
minimum relay fee, and can be used as an alternative way of sorting
transactions for mined blocks. Vuicoin Core will relay transactions with
...
entirely in Vuicoin Core 0.13, and it is at this time undecided whether the
...
Vuicoin Core automatically creates a hidden service to listen on, without
manual configuration. Vuicoin Core will also use Tor automatically to connect
to other .onion nodes if the control socket can be successfully opened. This
will positively affect the number of available .onion nodes and their usage.

This new feature is enabled by default if Vuicoin Core is listening, and
...
Vuicoind can now (optionally) asynchronously notify clients through a
...
(or `settxfee <n>` rpc during runtime). A value of `n=0` signals Vuicoin
Core to use floating fees. By default, Vuicoin Core will use floating
fees.
...
(default: `0.0002` VUI/kB).

At all times, Vuicoin Core will cap fees at `-maxtxfee=<x>` (default:
0.10) VUI.
Furthermore, Vuicoin Core will never create transactions paying less than
...
`vuicoin.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
Then, to tunnel a SSL connection on 26788 to a RPC server bound on localhost on port 19334 do:

    stunnel -d 26788 -r 127.0.0.1:19334 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /vuicoinrpc>
        ...
        # optional bypass vuicoind rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< vuicoinrpc:<password>
    </Location>
...
Restarting vuicoind will no longer clear out the list of banned peers; instead
...
- Adding `dbcache=100` in vuicoin.conf
...
For example:
    ...
    $ src/vuicoin-cli -stdin walletpassphrase
    mysecretcode
    120
...
Various code modernizations have been done. The Vuicoin Core code base has
...
- `vuicoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries for the most
  common 32-bit ARM architecture.
- `vuicoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries for the most
...
for a new opcode in the Vuicoin scripting system that in combination with
...
Vuicoin transactions currently may specify a locktime indicating when
...
HD wallets are incompatible with older versions of Vuicoin Core.
...
The two phases are distinct in the Vuicoin-Qt GUI. During the first one,
...
- **Elimination of unwanted transaction malleability:** Segregating the witness
  ...
  malleability, which is a problem that makes programming Vuicoin wallet
  software more difficult and which seriously complicates the design of smart
  contracts for Vuicoin.
...
- **Weighting data based on how it affects node performance:** Some parts of
  each Vuicoin block need to be stored by nodes in order to validate future
...
- **Signature covers value:** A simple improvement in the way signatures are
  ...
  of vuicoins they think they are spending, and by having full nodes refuse to
  accept those signatures unless the amount of vuicoins being spent is exactly
...
- **Increased security for multisig:** Vuicoin addresses (both P2PKH addresses
  ...
  believed to be provided by Vuicoin's choice of parameters for its Elliptic
...
- **More efficient almost-full-node security** Satoshi Nakamoto's original
  ...
  produce an accurate copy of Vuicoin's current ledger (called the UTXO set),
  ...
  note that Vuicoin Core does not provide an option to use this capability as
  of this 0.13.2 release.

- **Script versioning:** Segwit makes it easy for future soft forks to allow
  Vuicoin users to individually opt-in to almost any change in the Vuicoin
  Script language when those users receive new transactions.  Features
  currently being researched by Bitcoin and Vuicoin Core contributors that may
...
segwit's start date of 1 January 2017 miners can update the Vuicoin
client to Vuicoin Core 0.13.2 to signal for segwit support. When a
...
Since Vuicoin Core 0.10.0, nodes have defaulted to only relaying and
...
- `vuicoin-${VERSION}-arm-linux-gnueabihf.tar.gz`: Linux binaries targeting
  the 32-bit ARMv7-A architecture.
- `vuicoin-${VERSION}-aarch64-linux-gnu.tar.gz`: Linux binaries targeting

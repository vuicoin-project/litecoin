...
- [Development guidelines](#development-guidelines)
    - [General Vuicoin Core](#general-vuicoin-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Vuicoin Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play vuicoins" on the test network, if you
...
Vuicoin Core is a multi-threaded application, and deadlocks or other
...
([`valgrind.supp`](https://github.com/vuicoin-project/vuicoin/blob/master/contrib/valgrind.supp))
...
To profile a running vuicoind process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep vuicoind` -- sleep 60
```
...
Vuicoin Core can be compiled with various "sanitizers" enabled, which add
...
Some of these are maintained by active developers of Vuicoin Core, in which case changes should probably go
...
file descriptors, it will cause problems with Vuicoin's `select()` loop, because
...
compatibility between nodes. This happened in Vuicoin 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Vuicoin's
